...
std::vector<MediaCodecListBuilderBase *> GetBuilders() {
    ...
    if (surfaceTest == nullptr) {
        ALOGW("Allowing all OMX codecs");
        builders.push_back(&sOmxInfoBuilder);
    } else {
        ALOGW("Allowing only non-surface-encoder OMX codecs");
...
void *MediaCodecList::profilerThreadWrapper(void * /*arg*/) {
    ALOGW("Enter profilerThreadWrapper.");
    ...
    ALOGW("Codec profiling started.");
    profileCodecs(infos, kProfilingResults);
    ALOGW("Codec profiling completed.");
    ...
    if (sCodecList == nullptr) {
        ...
        if (codecList->initCheck() == OK) {
            ...
            if (isProfilingNeeded()) {
                ALOGW("Codec profiling needed, will be run in separated thread.");
                pthread_t profiler;
...
MediaCodecList::MediaCodecList(std::vector<MediaCodecListBuilderBase*> builders) {
    ...
    for (MediaCodecListBuilderBase *builder : builders) {
        if (builder == nullptr) {
            ALOGW("ignored a null builder");
            continue;
        }
        auto currentCheck = builder->buildMediaCodecList(&writer);
        if (currentCheck != OK) {
            ALOGW("ignored failed builder");
            continue;
        } else {
...
void MediaCodecList::findMatchingCodecs(
    ...
    for (;;) {
        ...
        ALOGW("Iceows codec try '%s'", componentName.c_str());

        if (!codecHandlesFormat(mime, info, format)) {
            ALOGW("skipping codec '%s' which doesn't satisfy format %s",
                  componentName.c_str(), format->debugString(2).c_str());
            continue;
        }

        if ((flags & kHardwareCodecsOnly) && isSoftwareCodec(componentName)) {
            ALOGW("skipping SW codec '%s'", componentName.c_str());
            continue;
        }

        matches->push(componentName);
        ALOGW("matching '%s'", componentName.c_str());
    }
...
bool MediaCodecList::codecHandlesFormat(const char *mime, sp<MediaCodecInfo> info,
    ...
    if (format == nullptr) {
        ALOGW("codecHandlesFormat: no format, so no extra checks");
        return true;
    }
    ...
    if (capabilities == nullptr) {
        ALOGW("codecHandlesFormat: no capabilities for refinement");
        return true;
    }
    ...
    if (isVideo) {
        ...
        if (format->findInt32("height", &height) && format->findInt32("width", &width)) {
            ...
            if (width < minW || width > maxW || height < minH || height > maxH) {
                ALOGW("format %dx%d outside of allowed %dx%d-%dx%d",
                ...
                if (splitString(alignment, "x", &alignWidth, &alignHeight) ||
                    ...
                    if (wAlign > 0 && hAlign > 0) {
                         if ((width % wAlign) != 0 || (height % hAlign) != 0) {
                            ALOGW("format dimensions %dx%d not aligned to %dx%d",
        ...
        if (format->findInt32("profile", &profile)) {
            ...
            ALOGW("Codec support force profile %d with level %d", profile, level);

           /*
            if (it == profileLevels.end()) {
                ALOGW("Codec does not support profile %d with level %d", profile, level);
                return false;
            }
            */
        }
